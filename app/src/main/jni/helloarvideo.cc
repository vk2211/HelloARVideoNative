...
extern "C" {
    ...
    JNIEXPORT void JNICALL JNIFUNCTION_NATIVE(nativeAdd(JNIEnv* env, jobject object, jstring s1, jstring s2));
};
bool jstringToString(JNIEnv* env, jstring jstr, std::string &stlStr);

std::string namestr[10];
        ...
        if (!tracked_target) {
            if (video == NULL) {
                ...
                if(frame.targets()[0].target().name() == std::string("argame") && texid[0]) {
                    ...
                    int i = 3;
                    for (i = 3; i < index; i++) {
                        LOGI("############### else, target=%s, namestr[i]=%s", frame.targets()[0].target().name(), namestr[i].c_str());
...
JNIEXPORT void JNICALL JNIFUNCTION_NATIVE(nativeAdd(JNIEnv*env, jobject, jstring s1, jstring s2))
{
    std::string str1;
    std::string str2;
